...
class FiDT5(transformers.T5ForConditionalGeneration):
    def __init__(self, config):
        ...
        self.wrap_encoder()  # 调用wrap_encoder -> 调用EncoderWrapper.forward()
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs):
        if input_ids != None:
            ...
            input_ids = input_ids.view(input_ids.size(0), -1)  # [bsz, n_passages * passage_length] @ ren ma 2022.10.25
        if attention_mask != None:
            attention_mask = attention_mask.view(attention_mask.size(0), -1) # [bsz, n_passages * passage_length]  @ ren ma 2022.10.25
        return super().forward(
            input_ids=input_ids,
            attention_mask=attention_mask,
             **kwargs  # 进入modeling_t5.py/T5ForConditionalGeneration.forward()  @ ren ma 2022.10.25
        )
...
class EncoderWrapper(torch.nn.Module):
    ...
    def forward(self, input_ids=None, attention_mask=None, **kwargs,):
        """主要把input_ids从[bsz，n_passages * passage_length](多个passages拼接)转为
        [bsz*n_passages, passage_length](每个passage独立)，经过模型输出outputs.last_hidden_state，
        再转回[bsz，n_passages * passage_length](每个passage独立经过encoder后再拼接到一起)"""
        ...
        # input_ids.shape转为[bsz*n_passages, passage_length]，因为bsz为1，相当于[n_passages, passage_length]，
        # 在进入模型时默认n_passages为batch_size，如此把每个passage当作一个单独的输入进入encoder。
        outputs = self.encoder(input_ids, attention_mask, **kwargs)  # 进入modeling_t5.py/T5Stack.forward()
        # 此时outputs.last_hidden_state.shape为[n_passages, passage_length, 768]
        # outputs = (outputs[0].view(bsz, self.n_passages*passage_length, -1), ) + outputs[1:]
        outputs.last_hidden_state = outputs.last_hidden_state.view(bsz, self.n_passages*passage_length, -1)
        # outputs.last_hidden_state.shape转为[bsz，n_passages * passage_length, 768]
        return outputs
...
class CheckpointWrapper(torch.nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, position_bias, **kwargs):
        if self.use_checkpoint and self.training:
            ...
            output = self.module(hidden_states, attention_mask, position_bias, **kwargs) # 进入modeling_t5.py/T5Block.forward()  @ ren ma 2022.10.25
        return output
